...
function innersect(w1, w2, kleene, eq) {
    // both are either empty or contain a wildcard
    if ((w1.length === 0 || w1 === kleene) &&
        (w2.length === 0 || w2 === kleene)) return true;
    ...
    if (eq(c1, c2) && innersect(remain1, remain2, kleene, eq)) return true;
    // if either is a wildcard and either remaining innersects with the other whole
    if ((c1 === kleene || c2 === kleene) && 
        (innersect(w1, remain2, kleene, eq) || 
         innersect(remain1, w2, kleene, eq))) return true;
...
function eqSegment(s1, s2) {
    return innersect(s1, s2, '*', eqChar);
}

// Usable export, specialized for paths
function intersect(w1, w2) {
    return innersect(w1.split('/'), w2.split('/'), '**', eqSegment);
}
